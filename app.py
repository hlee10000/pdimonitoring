...
if uploaded_files:
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        ...
        st.subheader(f"📷 {idx+1}번째 확대된 원본 이미지")
        ...
        st.subheader(f"📊 {idx+1}번째 단일 행 데이터 검수")
        matrix = {i: [digits[i-1]] for i in range(1, 32)}
        if has_total: matrix["Total"] = [total_val]
        df = pd.DataFrame(matrix)
        
        # data_editor에 고유 key 부여
        edited_df = st.data_editor(df, use_container_width=True, key=f"edit_{idx}")
